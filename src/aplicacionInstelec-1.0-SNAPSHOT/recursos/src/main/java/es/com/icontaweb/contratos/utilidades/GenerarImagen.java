...
public class GenerarImagen {
     ...
     */
    private static void generateSignature(String jsonEncoding, OutputStream output) throws IOException {
    ...
    public byte[] ImagenJson(String JsonString) {

        ServletContext servletContext = (ServletContext) FacesContext.getCurrentInstance().getExternalContext().getContext();
        String path = servletContext.getRealPath("/") + "/recursos/imgfirma.png";
        
        byte[] resultado = null;
        try {
            resultado = redrawSignature(extractSignature(JsonString));
    ...
    public byte[] Imagen64(String JsonString) {

        String encoded = JsonString.substring(URL_DATA_PNG_BASE64_PREFIX.length());
