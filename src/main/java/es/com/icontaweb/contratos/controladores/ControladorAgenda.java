...
public class ControladorAgenda implements Serializable {
    ...
    private double cuentaAgenda;

    public double getCuentaAgenda() {

        cuentaAgenda = 0;
        
        em = entityManager();
        try {
            String sqlquery = "select a from Agenda a ";
            sqlquery = sqlquery + "where a.fechaProxima <= CURRENT_DATE ";

            Query lista = em.createQuery(sqlquery);
            cuentaAgenda = lista.getResultList().size();
        } catch (Exception e) {
            System.out.println("CONTRATOS " + (new Date()).toString() + " " + e.getMessage());
        } finally {
            if (em.isOpen()) em.close();
    ...
    public void setCuentaAgenda(double cuentaAgenda) {
        this.cuentaAgenda = cuentaAgenda;
    }
